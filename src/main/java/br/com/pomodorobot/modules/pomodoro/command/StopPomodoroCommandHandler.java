...
public class StopPomodoroCommandHandler implements PomodoroSubcommandHandler {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        PomodoroSessionResponse response = pomodoroSessionService.stop(
                PomodoroCommandSupport.guildId(event),
                PomodoroCommandSupport.userId(event)
        );

        pomodoroMainMessageService.update(response);
        event.replyEmbeds(PomodoroEmbedFactory.stopped(response))
